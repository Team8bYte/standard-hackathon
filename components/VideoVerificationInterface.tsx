...
export default function VideoVerificationInterface() {
  ...
  const handleStepComplete = () => {
    if (currentStep < applicationSteps.length) {
      ...
      setTimeout(() => {
        const sectionElement = document.getElementById(`step-section-${nextStep}`);
        if (sectionElement) {
  ...
  return (
    <div className="container mx-auto py-8 max-w-6xl">
        ...
        <div className="flex flex-col space-y-2">
          <div className="flex items-center justify-between">
            {applicationSteps.map((step, index) => {
              const isClickable = verificationComplete && step.id <= currentStep;
              ...
              return (
                <div 
                  key={step.id} 
                  className={`flex items-center ${isClickable ? 'group' : ''} relative`}
                  onClick={() => navigateToStep(step.id)}
                  style={{ cursor: isClickable ? 'pointer' : 'default' }}
                  title={isClickable ? `Go to ${step.name}` : step.disabled ? 'Complete previous steps first' : ''}
                  ref={(el) => {
                ...
                >
                  <div 
                    className={`flex items-center justify-center w-12 h-12 rounded-full transition-all duration-200 ${
                      isCurrentStep ? 'bg-primary text-primary-foreground shadow-lg' : 
                      currentStep > step.id ? 'bg-green-500 text-white' : 'bg-muted text-muted-foreground'
                    } ${isClickable && !isCurrentStep ? 'group-hover:ring-2 group-hover:ring-primary/50 group-hover:shadow-md' : ''}
                    ${isClicked ? 'scale-90 opacity-80' : ''}`}
                  >
                    <step.icon className={`w-6 h-6 ${isClickable ? 'group-hover:scale-110 transition-transform duration-200' : ''}`} />
                  ...
                  {index < applicationSteps.length - 1 && (
                    <div className={`w-24 h-1 mx-4 transition-colors duration-200 ${
      ...
      {!userAuthenticated ? (
        <div className={`grid grid-cols-1 md:grid-cols-2 gap-6 ${getSectionAnimationClass(1)}`} id="step-section-1">
        ...
        <>
          ...
          <div className={`p-6 border rounded-lg bg-green-50 mb-6 ${getSectionAnimationClass(1)}`} id="step-section-1">
          ...
          {currentStep === 2 && (
            <div className={`mt-8 ${getSectionAnimationClass(2)}`} id="step-section-2">
          ...
          {currentStep === 3 && (
            <div className={`mt-8 ${getSectionAnimationClass(3)}`} id="step-section-3">
          ...
          {currentStep === 4 && (
            <div className={`mt-8 ${getSectionAnimationClass(4)}`} id="step-section-4">
              <SubmitApplication />
            </div>
          )}
        </>
