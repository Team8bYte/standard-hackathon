...
import { Upload, FileCheck, AlertTriangle, Trash2, RefreshCw } from "lucide-react"
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogContent,
...
export default function DocumentVerification({ onComplete }: DocumentVerificationProps) {
  ...
  const checkCompletion = useCallback(() => {
    const allDocumentsVerified = REQUIRED_DOCUMENTS.every(required =>
      documents.some(doc => doc.type === required.type && doc.status === 'verified')
    );
  ...
  const handleFileUpload = async (type: string, file: File) => {
    ...
    try {
      ...
      setDocuments(prev => prev.map(doc =>
        doc.id === id ? { ...doc, status: 'failed', error: 'Verification failed' } : doc
      ));
                  ...
                  <div key={doc.type} className="p-4 border rounded-lg">
                    <div className="flex justify-between items-start">
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground mt-1">Accepted formats: {doc.formats}</p>
                      </div>
                    ...
                    {uploadedDoc && (
                      <div className="mt-4 bg-muted/30 p-3 rounded-md">
                        ...
                        {uploadedDoc.status === 'processing' && (
                          <p className="text-xs text-muted-foreground mt-2">Verifying document...</p>
                        )}
                        {uploadedDoc.status === 'failed' && uploadedDoc.error && (
                          <p className="text-xs text-red-500 mt-2">{uploadedDoc.error}</p>
                        )}
                      </div>
                    )}
                  </div>
