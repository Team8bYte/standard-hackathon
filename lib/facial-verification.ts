...
async function calculateSimilarity(descriptor1: Float32Array, descriptor2: Float32Array): Promise<number> {
  try {
...
export async function extractFaceDescriptor(imageElement: HTMLImageElement): Promise<Float32Array | null> {
  ...
  try {
    ...
    console.log('Detecting face...', 'Image dimensions:', imageElement.width, 'x', imageElement.height);
...
export function calculateMatchScore(descriptor1: Float32Array, descriptor2: Float32Array): number {
 ...
 */
export function dispatchVerificationEvent(action: string, success: boolean, userData: any = null): void {
  try {
    ...
    console.log(`Dispatched ${action} verification event:`, { success, userData });
  } catch (error) {
...
export async function findMatchingFace(descriptor: Float32Array): Promise<MatchResult | null> {
  try {
    ...
    for (const storedFace of storedFaces) {
      ...
      const score = await calculateSimilarity(descriptor, storedFace.descriptor);
...
export function listStoredFaces(): Array<{id: string, userData: UserData, createdAt: string}> {
...
export async function extractFaceDescriptorWithOptions(
  ...
  try {
    ...
    console.log(`Detecting face with options: minConfidence=${minConfidence}, enlargeFactor=${enlargeFactor}`);
